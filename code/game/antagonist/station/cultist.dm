...
/datum/antagonist/cultist/add_antagonist(datum/mind/player, ignore_role, do_not_equip, move_to_spawn, do_not_announce, preserve_appearance, forced)
	. = ..()
	if(.)
		to_chat(player, span_cult("[conversion_blurb]"))
...
/datum/antagonist/cultist/proc/update_cult_magic(list/to_update)
	if(CULT_RUNES_1 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, span_cult("The veil between this world and beyond grows thin, and your power grows."))
				add_cult_magic(H.current)
	if(CULT_RUNES_2 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, span_cult("You feel that the fabric of reality is tearing."))
				add_cult_magic(H.current)
	if(CULT_RUNES_3 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, span_cult("The world is at end. The veil is as thin as ever."))
				add_cult_magic(H.current)
...
/datum/antagonist/cultist/proc/offer_uncult(mob/M)
	...
	to_chat(M, span_cult("Do you want to abandon the cult of Nar'Sie? <a href='?src=\ref[src];confirmleave=1'>ACCEPT</a>"))
