...
/obj/MouseDrop_T(atom/dropped, mob/living/user)
	...
	if (dropped != src && HAS_FLAGS(obj_flags, OBJ_FLAG_RECEIVE_TABLE) && isobj(dropped))
		...
		if (HAS_FLAGS(object.obj_flags, OBJ_FLAG_CAN_TABLE))
			if (object.anchored)
				USE_FEEDBACK_FAILURE("[object] is firmly anchored and cannot be moved.")
				return TRUE
			if (!isturf(loc))
				USE_FEEDBACK_FAILURE("[src] must be on a turf to lift [dropped] onto it.")
				return TRUE
			if (!user.skill_check(SKILL_HAULING, SKILL_BASIC))
				USE_FEEDBACK_FAILURE("You're not strong enough to lift [dropped] onto [src].")
				return TRUE
			...
			if (!HAS_FLAGS(obj_flags, OBJ_FLAG_RECEIVE_TABLE))
				...
				return TRUE
			if (!HAS_FLAGS(object.obj_flags, OBJ_FLAG_CAN_TABLE))
				USE_FEEDBACK_FAILURE("[dropped]'s state has changed.")
				return TRUE
			if (object.anchored)
				USE_FEEDBACK_FAILURE("[object] is firmly anchored and cannot be moved.")
				return TRUE
...
/obj/proc/can_anchor(obj/item/tool, mob/user, silent = FALSE)
	if (isinspace())
		if (!silent)
			USE_FEEDBACK_FAILURE("[src] cannot be anchored in space.")
		return FALSE
	return TRUE
