...
/proc/medical_scan_results(mob/living/carbon/human/H, verbose, skill_level = SKILL_DEFAULT)
		...
		if(!brain || H.stat == DEAD || (H.status_flags & FAKEDEATH))
			...
			if(H.has_brain_worms())
				...
				if(skill_level < SKILL_BASIC)
					...
					brain_result = span_danger("ERROR - Organ not recognized")
	else
		brain_result = span_danger("ERROR - Nonstandard biology")
	dat += "Brain activity: [brain_result]."

	if(H.stat == DEAD || (H.status_flags & FAKEDEATH))
		dat += span_warning("[b]Time of Death:[endb] [time2text(worldtime2stationtime(H.timeofdeath), "hh:mm")]")
	...
	if(H.should_have_organ(BP_HEART))
		...
		if(H.pulse() == PULSE_NONE)
			pulse_result = span_danger("[pulse_result]")
		else if(H.pulse() < PULSE_NORM)
			pulse_result = span_notice("[pulse_result]")
		else if(H.pulse() > PULSE_NORM)
			pulse_result = span_warning("[pulse_result]")
	else
	...
	if(H.should_have_organ(BP_HEART))
		...
		switch(H.get_blood_oxygenation())
			if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
				oxygenation_string = span_notice("[oxygenation_string]")
			if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_OKAY)
				oxygenation_string = span_warning("[oxygenation_string]")
			if(-(INFINITY) to BLOOD_VOLUME_SURVIVE)
				oxygenation_string = span_danger("[oxygenation_string]")
	...
	switch(H.radiation)
		...
		if(1 to 30)
			dat += span_notice("Patient shows minor traces of radiation exposure.")
		if(31 to 60)
			dat += span_notice("Patient is suffering from mild radiation poisoning.")
		if(61 to 90)
			dat += span_warning("Patient is suffering from advanced radiation poisoning.")
		if(91 to 120)
			dat += span_warning("Patient is suffering from severe radiation poisoning.")
		if(121 to 240)
			dat += span_danger("Patient is suffering from extreme radiation poisoning. Immediate treatment recommended.")
		if(241 to INFINITY)
			dat += span_danger("Patient is suffering from acute radiation poisoning. Immediate treatment recommended.")

	// Traumatic shock.
	if(H.is_asystole())
		dat += span_danger("Patient is suffering from cardiovascular shock. Administer CPR immediately.")
	else if(H.shock_stage > 80)
		dat += span_warning("Patient is at serious risk of going into shock. Pain relief recommended.")
		...
		for(var/name in H.organs_by_name)
			...
			if(e.status & ORGAN_BROKEN)
				if(((e.name == BP_L_ARM) || (e.name == BP_R_ARM) || (e.name == BP_L_LEG) || (e.name == BP_R_LEG)) && (!e.splinted))
					dat += span_warning("Unsecured fracture in subject [limb]. Splinting recommended for transport.")
			if(e.has_infected_wound())
				dat += span_warning("Infected wound detected in subject [limb]. Disinfection recommended.")
		...
		for(var/obj/item/organ/external/e in H.organs)
			if(e)
				if(!found_disloc && e.dislocated >= 1)
					dat += span_warning("Dislocation detected. Advanced scanner required for location.")
					found_disloc = TRUE
				if(!found_bleed && (e.status & ORGAN_ARTERY_CUT))
					dat += span_warning("Arterial bleeding detected. Advanced scanner required for location.")
					found_bleed = TRUE
				if(!found_tendon && (e.status & ORGAN_TENDON_CUT))
					dat += span_warning("Tendon or ligament damage detected. Advanced scanner required for location.")
					found_tendon = TRUE
		...
		if(length(damaged))
			for(var/obj/item/organ/external/org in damaged)
				...
				if(org.status & ORGAN_BLEEDING)
					limb_result = "[limb_result] \[[span_danger("bleeding")]\]"
				dat += limb_result
		else
	...
	if(H.reagents.total_volume)
		...
		if(unknown)
			print_reagent_default_message = FALSE
			. += span_warning("Warning: Unknown substance[(unknown>1)?"s":""] detected in subject's blood.")
	...
	if(ingested && ingested.total_volume)
		...
		if(unknown)
			print_reagent_default_message = FALSE
			. += span_warning("Non-medical reagent[(unknown > 1)?"s":""] found in subject's stomach.")
