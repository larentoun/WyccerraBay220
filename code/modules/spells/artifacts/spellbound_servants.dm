...
/datum/spellbound_type/apprentice/set_antag(datum/mind/M, mob/master)
	GLOB.wizards.add_antagonist_mind(M, TRUE, ANTAG_APPRENTICE, "<b>You are an apprentice-type Servant! You're just an ordinary Wizard-To-Be, with no special abilities, but do not need robes to cast spells. Follow your teacher's orders!</b>", forced = TRUE)
...
/datum/spellbound_type/servant/set_antag(datum/mind/M, mob/master)
	GLOB.wizards.add_antagonist_mind(M, TRUE, ANTAG_SERVANT, "<b>You are a [name]-type Servant!</b> [spiel]", forced = TRUE)
