...
/obj/item/laserframe/examine(mob/user)
	...
	switch(buildstate)
		if(1)
			. += span_notice("It has a pipe segment installed.")
		if(2)
			. += span_notice("It has a pipe segment taperolled.")
		if(3)
			. += span_notice("It has an emitter installed.")
		if(4)
			. += span_notice("It has a powerccell installed.")
		if(5)
			. += span_notice("It has a capacitor installed.")
		if(6)
			. += span_notice("It has a firing mechanism installed.")
		if(7)
			. += span_notice("It has an igniter installed.")
		if(8)
			. += span_notice("It has an electronic frame installed.")
		if(9)
			. += span_notice("It has a reserve powercell installed.")
		if(10)
			. += span_notice("It has a firing processor installed.")
		if(11)
			. += span_notice("It has a weapon board installed.")
		if(12)
			. += span_notice("It has a capacitor-retranslator installed.")
		if(13)
			. += span_notice("It has cable coils installed.")
		if(14)
			. += span_notice("It has a cable welded into place.")
		if(15)
			. += span_notice("It has a little part of shitcode inside.")
		if(16)
			. += span_notice("It has lenses and a scope.")
		if(17)
			. += span_notice("It has processed and calibrated lenses.")
	if(ishuman(user))
		if(user.skill_check(SKILL_DEVICES, SKILL_EXPERIENCED))
			switch(buildstate)
				if(0)
					. += span_notice("Looks like you need something as lenses frame... A pipe?")
				if(1)
					. += span_notice("The pipe is unsecured - fix it with tape roll.")
				if(2)
					. += span_notice("You some little laser, for start. A high-power micro-laser shouldn't melt the frame and be powerful enough.")
				if(3)
					. += span_notice("You need a power storage... A hive power cell will be enough.")
				if(4)
					. += span_notice("The energy should be concetrated in capacitor before firing. Advanced one should be enough.")
				if(5)
					. += span_notice("It need firing mechanism. Steel one would fit perfectly...")
				if(6)
					. += span_notice("I don't remember why, but igniter is necessary here.")
				if(7)
					. += span_notice("It need a frame for circuits, plastic one.")
				if(8)
					. += span_notice("It need a device power cell for idle capacity.")
				if(9)
					. += span_notice("It will not work without circuits. A small processor unit from some PDA should be enough.")
				if(10)
					. += span_notice("I need a power control circuit here - otherwise, it will explode in my hands after second shoot....")
				if(11)
					. += span_notice("A standard superconductive magnetic coil need here.")
				if(12)
					. += span_notice("It cannot work without cables...")
				if(13)
					. += span_notice("I need to weld it together.")
				if(14)
					. += span_notice("It need calibration, circuits frequencies should be messed up.")
				if(15)
					. += span_notice("It need lenses for firepower... And scope. Yeap, I wanna scope here.")
				if(16)
					. += span_notice("Lenses should be calibrated. I don't know how to do that... RCD may help.")
				if(17)
					. += span_notice("A screwdriver should secure it all.")
